 ...
 * 文件格式：每个任务以独占一行的 [任务名] 开头，直到下一个 [任务名] 或文件末尾；
 * # 开头为注释行；只覆盖文件中出现的任务，缺失的任务回退到内置默认提示词。
 */
const PROMPTS_FILE_RELATIVE_PATH = path.join("public", "prompts.txt");

export function parsePromptsFile(content: string): Partial<Record<Task, string>> {
  const result: Partial<Record<Task, string>> = {};
  const headerPattern = /^\[([a-z]+)\]\s*$/gm;
  let match: RegExpExecArray | null;
  let lastKey: string | null = null;
  let lastIndex = 0;
  while ((match = headerPattern.exec(content)) !== null) {
    if (lastKey && lastKey in taskInstructions) {
      const value = content.slice(lastIndex, match.index).trim();
      if (value) result[lastKey as Task] = value;
    }
    lastKey = match[1];
...
let promptsCache: Record<Task, string> | undefined;
...
export function loadTaskInstructions(
  ...
  try {
    ...
    const merged: Record<Task, string> = { ...taskInstructions };
