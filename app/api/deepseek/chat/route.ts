...
const taskInstructions: Record<Task, string> = {
  translate: "仅将“用户选中内容”译为自然、准确、可直接阅读的中文，直接输出译文。禁止输出开场白、结束语、总结、评价、原文复述或代码块；相邻上下文仅供理解，不得翻译或引用。若用户选中多个片段，按原文顺序逐段输出译文，每段前标注“# 1”“# 2”等，片段间空行分隔。忠实保持原文结构、语气与证据链；数值、公式、单位、引用编号、模型名、数据集名、算法名、API 名、变量名和缩写一律原样保留；数学公式尽量用 $...$ 或 $$...$$ 包裹。计算机专业术语优先采用学界通用中文译名，首次出现可括注英文，同一术语保持统一；不得补充原文没有的论断。若原文为单句或段落，在译文后增加语法解释教学。",
