...
- 支持缩放、平移、页码跳转与章节目录；`Ctrl`/`Cmd`+滚轮缩放保持阅读位置稳定。
...
- 内置 10 套阅读主题：经典原版、类纸护眼白、豆沙绿护眼、羊皮纸复古、深色护眼、赛博朋克、极简黑白、深蓝学术、莫兰迪灰、高贵典雅。
...
- 三提供方：DeepSeek、免费智谱 GLM 4.7 Flash 与 Kimi（kimi-k2.6），各有独立 API Key 与连接验证；DeepSeek 与 Kimi 支持并发对话，GLM 免费档保持单任务。
...
- 更新：再次双击 `一键安装.bat`，自动检测旧版本并覆盖安装，保留 `data` 数据。
...
- **覆盖安装（有旧版本）**：再次双击 `一键安装.bat`，检测到 `%LOCALAPPDATA%\PaperMate\config.json` 中的已安装版本后，直接覆盖安装新版并保留 `data` 数据文件夹，无需重新选择安装位置（不再需要单独的升级脚本）。
...
| 界面 | 说明 |
| --- | --- |
| ![论文库首页](截图/首页.png) | 论文库：拖入 PDF、搜索论文、查看元数据与备注、拖动排序/置顶、查看备份状态。 |
| ![原版阅读与划选](截图/辅助阅读.png) | 原版页面阅读器：透明文本层精确划选，支持缩放、章节跳转和引用/图表链接。 |
| ![翻译与问答](截图/辅助阅读翻译.png) | 翻译选区、结合上下文解释、多轮问答，答案标注来源页码。 |
| ![设置](截图/设置.png) | 统一设置面板：模型提供方与 API Key 验证、阅读主题与完整 JSON 备份管理。 |
| ![皮肤选择](截图/皮肤选择.png) | 一键切换阅读主题，选择保存在本机。 |
| ![阅读笔记](截图/阅读笔记.png) | 生成的阅读笔记，含页码证据、术语表与可迁移启发。 |
| ![论文脑图](截图/论文脑图.png) | 可折叠的论文论证结构脑图，可导出 SVG。 |
| ![写作思路](截图/写作思路.png) | 写作策略分析，含论证链、章节职责与可迁移句式框架。 |
