...
- 双提供方：DeepSeek 与免费智谱 GLM 4.7 Flash，各有独立 API Key 与连接验证。
...
打开 `http://localhost:3000`，点击"设置"，输入 DeepSeek 或智谱 GLM 的 API Key 并验证连接。
...
1. 打开"设置"，选择 DeepSeek 或智谱 GLM，填写 API Key 并验证连接。
...
- 不支持 DOCX 等其他文档格式。
- 无账号体系与云端同步，数据仅保存在本机。
- 模型问答依赖 DeepSeek 或智谱 GLM 的 API Key 与网络连接。
