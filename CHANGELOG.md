...
- **Wheel-scrolling while drag-selecting discarded the selection.** Two places recomputed the selection from the pointer's **screen** coordinates while a drag was in progress. (1) Any scroll event (wheel, scrollbar, keyboard) recomputed the selection end, so once the content moved the text under the pointer was different and the accumulated selection was rewritten. (2) Committing and re-drawing used `start/end.clientY` — captured at pointer-down — to derive the vertical band that decides which rows were swept, so after a scroll those stale coordinates no longer matched any row and only the two anchor rows survived. Measured before the fix: selecting 4 rows, scrolling down 600 px and releasing left 2 highlighted rows (5 → 2); selecting 3 rows, scrolling up 600 px and releasing left 2 (3 → 2). Now scrolling only moves the viewport: the pending selection is left untouched (tracked with `selectionRef.scrolledSincePointerMove`), the end is recomputed only when the pointer actually moves, and on release the end recorded during the drag is used whenever a scroll happened in between. Each selection point also records the `scrollTop` at capture time, so committing and re-drawing shift `clientY` back into current screen coordinates before the band is evaluated. Re-measured after the fix, one fresh page load per scenario: no scrolling 5 → 5 rows; scroll down 600 px 5 → 5; scroll up 400 px 5 → 5; and "drag partway, wheel to the next page, keep dragging" extends to 39 rows / 4318 characters, so selecting beyond one screen still works. The now-unused `refreshActiveSelectionFromPointer()` and its two helper refs were removed (`components/pdf-reader.tsx`, `app/page.tsx`).
- **"Filling in paper info" fired on every open.** Opening a paper decided whether to re-parse the PDF / re-query journal metadata from fixed conditions, two of which could never become false: a PDF with no link annotations produced `links: undefined` (no distinction between "scanned, genuinely none" and "could not tell"), which JSON then dropped, so "every page is missing links" stayed true and the whole PDF was re-parsed on every open; and internal links whose target position cannot be resolved were retried forever. On top of that, "no impact factor → look it up" made the notice appear on every open even when nothing looked missing. The parser now reports an empty array for "scanned, no links" (leaving `undefined` for "could not tell"), and each paper keeps a backfill log (`linksCheckedAt` / `linksAttempts` / `metadataCheckedAt` / `metadataAttempts`): a failed backfill retries on a 24-hour cooldown and gives up after 5 consecutive failures, while a success clears the counter. Backfill now always runs in the background after the paper is already on screen, so opening is never blocked, and the notice says which of the two things it is doing (layout data vs. journal metadata) instead of "metadata and layout data". The decision and retry policy live in a pure module, `lib/paper-maintenance.ts`, with 14 new tests.

### Changed

- The `papers` table gained four auto-migrated columns for the backfill log (`links_checked_at`, `links_attempts`, `metadata_checked_at`, `metadata_attempts`). The migration is additive and guarded per column, so an existing library keeps every paper, workspace and note; the storage tests cover saving and reading the new fields after migrating an older database (`lib/storage.ts`, `lib/types.ts`).
- The release workflow's package check now asserts the same user-facing entries as the packager itself — `scripts/install-package.ps1`, `一键安装.bat` and `一键卸载.bat` — so a packaging regression fails the build rather than a user's install (`.github/workflows/release.yml`).
